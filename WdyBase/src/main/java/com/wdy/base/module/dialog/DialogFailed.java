 ...
 * 创建时间：2018/9/3.
 */
public class DialogFailed {
    @SuppressLint("StaticFieldLeak")
    private static DialogFailed mInstance;
    ...
    public static DialogFailed getInstance() {
        if (mInstance == null) {
            synchronized (DialogFailed.class) {
                if (mInstance == null) {
                    mInstance = new DialogFailed();
                }
            }
        }
        return mInstance;
    ...
    private void initDialog() {
        if (mAlertDialog == null) {
            ...
            layout = inflaterDl.inflate(R.layout.wdy_dialog_failed, null);
