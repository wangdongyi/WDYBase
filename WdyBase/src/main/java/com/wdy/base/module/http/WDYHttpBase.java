 ...
 * 创建时间：2019-05-23.
 */
public class WDYHttpBase {
    private OkHttpClient okHttpClient;
    private Gson gson;
    private volatile static WDYHttpBase wdyHttpBase;
    ...
    public static WDYHttpBase getInstance() {
        if (wdyHttpBase == null) {
            synchronized (WDYHttpBase.class) {
                if (wdyHttpBase == null) {
                    wdyHttpBase = new WDYHttpBase();
                }
            }
        }
        return wdyHttpBase;
    ...
    public void getEasyAsyncHttp(String url, final OnWDYEasyHttpCallback onWDYEasyHttpCallback) {
    ...
    public void postEasyAsyncHttp(Request request, final OnWDYEasyHttpCallback onWDYEasyHttpCallback) {
        okHttpClient.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(@NonNull final Call call, @NonNull final IOException e) {
                new android.os.Handler(Looper.getMainLooper()).post(new Runnable() {
    ...
    public void AsyncHttp(Request request, final OnWDYHttpCallback onWDYHttpCallback) {
        okHttpClient.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(@NonNull final Call call, @NonNull final IOException e) {
                new android.os.Handler(Looper.getMainLooper()).post(new Runnable() {
