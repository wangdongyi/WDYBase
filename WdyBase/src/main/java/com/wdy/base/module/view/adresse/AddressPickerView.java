...
public class AddressPickerView extends RelativeLayout implements View.OnClickListener {
    ...
    private int defaultSelectedColor = Color.parseColor("#FF4081");
    ...
    // 确定字体可以点击时候的颜色
    private int defaultSureCanClickColor = Color.parseColor("#FF4081");
    ...
    private List<ItemBean> mRvData; // 用来在recyclerview显示的数据
    private AddressAdapter mAdapter;   // recyclerview 的 adapter

    private YwpAddressBean mYwpAddressBean; // 总数据
    private ItemBean mSelectProvice = new ItemBean(); //选中 省份 bean
    private ItemBean mSelectCity = new ItemBean();//选中 城市  bean
    private ItemBean mSelectDistrict = new ItemBean();//选中 区县  bean
    ...
    private void initData() {
        String areaJson = readAssets(getContext(), "areacode.json");
        // 将数据转换为对象
        mYwpAddressBean = new Gson().fromJson(areaJson, YwpAddressBean.class);
        if (mYwpAddressBean != null) {
            mRvData.clear();
            for (int i = 0; i < mYwpAddressBean.getProvinces().size(); i++) {
                YwpAddressBean.ProvincesBean bean1 = mYwpAddressBean.getProvinces().get(i);
                ItemBean itemBean = new ItemBean();
                itemBean.setId(bean1.getId());
                itemBean.setName(bean1.getName());
    ...
    private void sure() {
        if (mSelectProvice != null &&
            ...
            if (mOnAddressPickerSureListener != null) {
                mOnAddressPickerSureListener.onSureClick(mSelectProvice.getName() + " " + mSelectCity.getName() + " " + mSelectDistrict.getName() + " ",
                        mSelectProvice.getId(), mSelectCity.getId(), mSelectDistrict.getId());
            }
        } else {
        ...
        public void onTabSelected(TabLayout.Tab tab) {
            ...
            switch (tab.getPosition()) {
                ...
                case 1:
                    ...
                    if (mSelectProvice != null) {
                        for (int i = 0; i < mYwpAddressBean.getProvinces().size(); i++) {
                            if (mYwpAddressBean.getProvinces().get(i).getId().equals(mSelectProvice.getId())) {
                                mRvData.clear();
                                for (int j = 0; j < mYwpAddressBean.getProvinces().get(i).getCities().size(); j++) {
                                    YwpAddressBean.ProvincesBean.CitiesBean citiesBean = mYwpAddressBean.getProvinces().get(i).getCities().get(j);
                                    ItemBean itemBean = new ItemBean();
                                    itemBean.setId(citiesBean.getId());
                                    itemBean.setName(citiesBean.getName());
                                    itemBean.setSelected(false);
                                    mRvData.add(itemBean);
                                }
                            }
                    ...
                    if (mSelectProvice != null && mSelectCity != null) {
                        for (int i = 0; i < mYwpAddressBean.getProvinces().size(); i++) {
                            if (mYwpAddressBean.getProvinces().get(i).getId().equals(mSelectProvice.getId())) {
                                ...
                                for (int j = 0; j < mYwpAddressBean.getProvinces().get(i).getCities().size(); j++) {
                                    YwpAddressBean.ProvincesBean.CitiesBean citiesBean = mYwpAddressBean.getProvinces().get(i).getCities().get(j);
                                    if (mSelectCity.getId().equals(citiesBean.getId())) {
                                        mRvData.clear();
                                        for (int k = 0; k < citiesBean.getCounties().size(); k++) {
                                            YwpAddressBean.ProvincesBean.CitiesBean.CountiesBean countiesBean = citiesBean.getCounties().get(k);
                                            ItemBean itemBean = new ItemBean();
                                            itemBean.setId(countiesBean.getId());
                                            itemBean.setName(countiesBean.getName());
                                            itemBean.setSelected(false);
                                            mRvData.add(itemBean);
                                        }
                                    }
        ...
        public void onBindViewHolder(final ViewHolder holder, final int position) {
            ...
            holder.mTitle.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    switch (tabSelectPosition) {
                        case 0:
                            mSelectProvice.setSelected(true);
                            mSelectProvice.setId(mRvData.get(position).getId());
                            mSelectProvice.setName(mRvData.get(position).getName());
                            // 清空后面两个的数据
                            mSelectCity = null;
                            mSelectDistrict = null;
